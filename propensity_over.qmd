...
Using the propensity scores generated from these population selection models, I generated a binary treatment assignment variable by comparing an individual's true propensity score to a random draw from a uniform distribution between 0 and 1. If the propensity score of an individual was higher than the random draw of the uniform distribution, the individual was assigned to treatment such that $Z = 1$, otherwise $Z = 0$. Because the intercept is calibrated, every population has a marginal treatment rate of 0.50, mirroring a balanced two-arm experiment and keeping treated and control sample sizes comparable across design cells. The strength of selection nevertheless varies by design: with coefficients drawn from a range that does not shrink with $p$, the variance of the true logit grows with the number of covariates, so the higher-dimensional conditions couple dimensionality with stronger selection and less overlap --- a bundle characteristic of high-dimensional administrative data, where richer covariate sets capture more of the selection process. I characterize each population's overlap alongside the results.
...
-   **Bagged CART**: bootstrapped aggregated classification trees using the R *ipred* package [@peters2009package]. I changed the default parameter to use 100 bootstrap replicates to align with the @Lee.2010u1 simulation. Propensity scores are the out-of-bag class probabilities, so each student's score is predicted only by trees whose bootstrap sample excluded that student; this avoids the overfitting that arises when ensemble predictions are taken on the training data itself.
...
-   **Weights**: a known issue with PSW is that extreme weights can result in biased treatment effects. Therefore, I examine the distribution of the estimated weights, including the maximum weight and the effective sample size implied by the weights [@Kish.1965]. I also report the ATE, its standard error, and coverage under weights truncated at the 1st and 99th percentiles [@Lee.2011tr].
...
In total, I tested three different covariate conditions (p = 20, 100, 200) with varying levels of complexity in the treatment and outcome models (Treatment population model = Base, Complex; Outcome population model = Base, Complex). I evaluated ten propensity score estimation methods: logit, CART, bagged CART, random forest, GBM, BART, Super Learner, NN-1, DNN-2, and DNN-3. The sample size was fixed at 10,000. By fully crossing all these conditions, I tested 120 scenarios; each simulated 1,000 times. One thousand replications matches or exceeds the comparison literature [@Setoguchi.2008ulg; @Cannas.2019] and was chosen with the precision of the coverage estimates in mind: the Monte Carlo standard error of an estimated coverage rate is at most $\pm 1.6$ percentage points (and $\pm 0.7$ at the nominal 95% level), while Monte Carlo error in the bias estimates is negligible relative to the effects of interest [@Morris.2019abs]. Monte Carlo uncertainty is reported alongside the results.
