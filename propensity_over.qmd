...
$X$ represents covariates and their associated coefficients ($\beta$). The output, $Pr(Z = 1|X)$ is the probability of assignment to the treatment condition (i.e., the propensity score, $e(x)$). The intercept, $\beta_0$, was set to 0.25, and the coefficients, $\beta_i$, were randomly assigned values between -0.4 and 0.4, which were based on the corresponding standardized coefficient values in the Common App evaluation; $\beta_i = 0$ for the quarter of covariates related only to the outcome, which do not enter the treatment model.
...
Using the propensity scores generated from these population selection models, I generated a binary treatment assignment variable by comparing an individual's true propensity score to a random draw from a uniform distribution between 0 and 1. If the propensity score of an individual was higher than the random draw of the uniform distribution, the individual was assigned to treatment such that $Z = 1$, otherwise $Z = 0$. Because the population coefficients are random draws, the marginal treatment rate varies across the fixed populations (from roughly 0.22 to 0.65 across the twelve design cells), as does the strength of selection: with coefficients drawn from a range that does not shrink with $p$, the variance of the true logit grows with the number of covariates, so the higher-dimensional conditions couple dimensionality with stronger selection and less overlap --- a bundle characteristic of high-dimensional administrative data, where richer covariate sets capture more of the selection process. I characterize each population's treatment rate and overlap alongside the results.
