---
title: "Evaluating Modern Propensity Score Estimation Methods with High-Dimensional Data"
...
source("code/ggplot_theme_pub.R")
...
With the rapid advancement of sophisticated machine learning algorithms, there are growing opportunities to develop and apply new methods for estimating propensity scores, particularly with high-dimensional data [@hernandez2019systematic]. These machine learning algorithms offer an automatic, data-driven way to capture non-linearities and non-additivity in the propensity score estimation model, potentially leading to a more balanced covariate distribution without manual iteration. Machine learning is now the most common alternative to logistic regression in applied propensity score work: a recent systematic review identified 179 applications across health, education, and the social sciences, with gradient boosting and random forests used most often --- and found that critical details such as hyperparameter settings and balance assessment frequently go unreported [@leite2025]. Yet the simulation evidence guiding these applications has focused on low-dimensional datasets [@Setoguchi.2008ulg; @Lee.2010u1; @Stuart.201027c; @Cannas.2019; @McCaffrey.2004pse]. Research evaluating these methods in genuinely high-dimensional data remains limited --- particularly under complexities in both the propensity and outcome models, and particularly with respect to the quality of the *inference* they support rather than point estimation alone.
...
In this study, we evaluate ten propensity score estimation methods --- logistic regression, a classification tree, bagged trees, random forest, gradient boosted trees, Bayesian additive regression trees, a Super Learner ensemble, and neural networks of one to three hidden layers --- for inverse probability weighting with high-dimensional data. The study makes three contributions. First, each method is evaluated as an applied researcher would actually encounter it, with configurations anchored to the defaults of the software packages education researchers use, rather than to study-specific tuning. Second, the simulated populations are generated from a reproducible framework for multi-type, correlated high-dimensional data whose parameters are calibrated to real administrative data from a large-scale education evaluation, at dimensionalities (up to 200 covariates) beyond those examined in the existing comparison literature. Third, beyond the bias and balance metrics that dominate prior comparisons, we evaluate the quality of the statistical inference each method supports --- confidence interval coverage and the calibration of estimated standard errors against true sampling variability --- which the applied literature rarely examines and the methodological literature increasingly identifies as the central weakness of machine-learning-based weighting [@leite2025].

The remainder of this article is structured as follows: First, we review the relevant literature for this study. Then, we describe our simulation approach and present our results. Finally, we conclude with a discussion section that focuses on the contributions of our study.

\newpage

## Causal Framework, Estimands, and Assumptions

### Neyman--Holland--Rubin Model for Causal Inference

The Neyman--Holland--Rubin model, also known as the Rubin causal model, forms the basis of our approach to causal inference [@rubin1976inference; @rubin2005causal; @holland1986statistics; @neyman1923application]. A core tenet of the Rubin causal model states that a treatment effect is the difference between two potential outcomes for an individual [@rubin2005causal]. Consider a dichotomous treatment variable ($Z$), where $Z_i = 1$ represents the $i$th student being in the treatment group---such as a college access program---and $Z_i = 0$ represents the student not being in the treatment group. Let $Y_{iz}$ be a potential outcome for student $i$ depending on treatment assignment $Z$, such that an individual student has two potential outcomes. $Y_{i1}$ is the potential outcome had student $i$ participated in the college access program, and $Y_{i0}$ is the potential outcome had that same student not participated. Note, the potential outcomes framework is based on the stable unit value assumption (SUTVA), which states there is only one version of the treatment condition and that the potential outcomes of an individual are not influenced by the treatment assignment of any other individual [@Rosenbaum.2010]. Therefore, the treatment effect of the program for student $i$ would be the difference in their two potential outcomes:
...
To balance the observed characteristics between treated and untreated students, iterating through various model specifications that include interactions and non-linear terms among the covariates is recommended [@Rosenbaum.2010]. However, this iterative process can become challenging and time-consuming as the number of available covariates increases, especially when conducting propensity score analysis with high-dimensional data. In pharmacoepidemiology, where administrative databases with hundreds of candidate covariates are routine, this challenge has produced a dedicated high-dimensional propensity score literature, including machine learning extensions for covariate prioritization and estimation [@karim2025hdps]; comparable guidance for the administrative datasets now common in education research remains scarce.
...
Ensemble methods are a family of algorithms that generate multiple trees to predict treatment assignment [@Lee.2010u1; @Stuart.2023; @Stuart.201027c]. By combining the predictions from multiple trees, ensemble methods can improve the accuracy of predictions since many weak trees together can create a better prediction than any single tree [@Lee.2010u1]. A popular ensemble method is bootstrapped aggregated CART, referred to as bagged-CART, which fits multiple CARTs on bootstrapped data samples [@Lee.2010u1]. Growing trees on bootstrapped samples reduce the chances of overfitting the data. Each tree will generate a probability that an individual will be assigned to treatment. The final probability of membership is based on aggregating the probabilities of assignment across all trees (i.e., forest). A more robust ensemble method is random forest [@Suk.20215xt]. Compared to CART and Bagging, the random forest algorithm grows trees by randomly selecting covariates and individuals to grow individual trees. With each new tree, the algorithm "learns" the best combination of covariates to generate a final probabilistic prediction based on the "forest" it has created. These random forest techniques have shown significant promise in predicting propensity scores in complex data associations [@Lee.2010u1; @Cannas.2019].

Two further ensemble strategies figure prominently in the propensity score literature and are included in this study. Gradient boosted trees build the ensemble sequentially, with each new tree fit to the prediction errors of the ensemble so far; introduced to propensity score estimation by @McCaffrey.2004pse, boosting has become the single most commonly used machine learning method in applied propensity score work [@leite2025]. Bayesian additive regression trees (BART) take a Bayesian approach to the same sum-of-trees idea, constraining each tree through a regularization prior to act as a weak learner [@chipman2010bart]; BART has an especially strong record in causal inference applications [@Hill.201183]. Finally, rather than choosing among candidate algorithms, the Super Learner combines them: it estimates an optimal weighted combination of a user-specified library of learners using cross-validated predictions [@vanderLaan2007], an approach shown to improve the robustness of propensity score estimation to model misspecification [@Pirracchio.201523m] and, in Monte Carlo scenarios calibrated to educational data specifically, to outperform its individual component learners [@autenrieth2021]. However, with the recent advances in computing power, non-tree-based methods, such as neural networks, have also increased in popularity [@collier2021estimating; @keller2015neural; @Westreich.2010; @Dreiseitl.2002tri; @Collier.2022].

\newpage

### Neural Network Approaches

Neural networks build predictions from layers of simple processing units. Each unit forms a weighted combination of its inputs and passes it through a non-linear activation function; a single hidden layer of such units, pooled by an output function, can already approximate complex non-linear and non-additive relationships between covariates and treatment assignment, and applying a logistic activation to the output layer bounds the predictions in [0, 1] so they can serve directly as propensity scores [@rosenblatt1958perceptron; @james2013introduction]. Deep neural networks (DNNs) extend this architecture with multiple hidden layers, each transforming the representations learned by the previous one, which allows increasingly complex functional forms to be approximated as data move through the network; in general, three hidden layers suffice for almost any functional form outside image and speech applications [@james2013introduction]. Appendix A provides a complete development of these architectures, from the single artificial neuron through the multi-layer network, with the associated notation and diagrams. A handful of studies have evaluated the performance of single-hidden-layer networks for estimating propensity scores in low-dimensional settings and found that they represent a suitable method, generally outperforming logistic regression in reducing bias and balancing covariates [@collier2021estimating; @keller2015neural; @Setoguchi.2008ulg; @Cannas.2019].

### Deep Neural Networks in Causal Inference

The popularity of DNNs in causal inference has increased in recent years due to their ability to uncover complex relationships in high-dimensional data with improved precision compared to traditional statistical and machine learning methods [@Pang.2019kdf; @hernandez2019systematic]. This capability to capture intricate relationships in large-volume data makes DNN algorithms suitable for applications in propensity score analysis and other causal inference methods. DNNs have garnered substantial attention in industry and academic research, making substantial advances in image recognition and natural language processing [@james2013introduction]; the large language models now pervasive in educational discourse are themselves deep neural networks. Mature high-level libraries such as Keras have also made DNN models straightforward to build and train without specialized engineering expertise [@Pang.2019kdf].
...
When this study was first designed, only two medical peer-reviewed studies had applied DNNs for propensity score estimation [@Whata.202204a; @Weberpals.2021lch]. @Weberpals.2021lch used data from around 130,000 cancer patients to construct a simulated dataset with 31 baseline covariates, where the treatment was a fictional cancer drug, and the outcome was a binary survival indicator. Employing a specific DNN architecture with three hidden layers, known as an "autoencoder", they found that DNN reduced confounding bias in treatment effect estimates compared to the traditional logistic model with manual variable selection. Conversely, @Whata.202204a compared the performance of logistic regression to a DNN with four hidden layers through a simulation (with 15 covariates) and a real-world application. In their simulation, they varied the level of complexity in the population selection model but not the outcome model. The results showed that DNN outperformed logistic regression regarding covariate balance, classification accuracy, and absolute relative bias.

Both studies demonstrate the growing promise in using DNNs for causal inference and suggest that DNNs may be a valid approach for estimating the propensity score.

The literature has continued to develop since. Most directly, @kim2026tradeoff compared deep and convolutional neural networks to logistic regression in a Monte Carlo study emphasizing high-dimensional covariates, and articulated what they call the classification--causal tradeoff: networks optimized too aggressively for predictive accuracy push propensity scores toward 0 and 1, destabilizing the resulting inverse probability weights. A complementary line of work responds by building covariate balance directly into network training --- for example, penalizing imbalance in the loss function itself [@shang2025calibration]. These developments sharpen rather than settle the question this study addresses: how the standard, prediction-trained networks that applied researchers can actually build today behave as propensity score estimators. Our study extends this literature in three ways: it compares DNNs not only to logistic regression but to the full roster of established machine learning propensity methods; it tests complexity in both the selection and outcome models with high-dimensional data calibrated to the administrative datasets encountered in educational research, a setting no prior DNN comparison has examined; and it evaluates the inferential properties --- coverage and standard error calibration --- that prior comparisons in this literature leave unexamined.

### Estimating Uncertainty After Propensity Score Weighting

A final strand of literature concerns what happens after the propensity scores are estimated: how the uncertainty of the weighted treatment effect should be quantified. Standard practice, embodied in the software applied researchers use, is to compute robust (sandwich) standard errors that treat the estimated weights as fixed and known [@robins2000marginal]. For the ATE with a correctly specified parametric propensity score model, this convention is well understood: ignoring the estimation of the weights yields standard errors that are, if anything, conservative [@lunceford2004; @kostouraki2024]. The guarantee is narrower than it appears, however. It does not extend to other estimands --- for the ATT, the same convention can be anti-conservative [@reifeis2022] --- and it does not extend to machine learning propensity scores at all, because the underlying theory assumes an estimating-equations first stage that flexible learners do not supply.

The evidence on what fixed-weight inference does under machine learning first stages is sobering. @naimi2023 showed in simulation that plugging machine learning estimators into singly robust methods such as inverse probability weighting can produce substantial bias and confidence intervals with far-below-nominal coverage, problems that persisted even when the machine learning predictions themselves were accurate; @balzer2023, in an invited commentary, emphasized that whether a given machine learning workflow supports valid inference is context-specific and called for simulation studies tailored to the settings where these methods are actually deployed --- precisely the role the present study plays for high-dimensional education data. On the variance-estimation side, @austin2022boot found that asymptotic standard errors for weighted estimators perform adequately at large sample sizes but are outperformed by bootstrapping at moderate ones, positioning the bootstrap as the general-purpose remedy when analytic corrections are unavailable.

Against this backdrop, a notable feature of the propensity score comparison literature is how rarely it engages inference at all. The benchmark simulations that established the machine learning methods evaluated here report bias, mean squared error, and covariate balance [@Setoguchi.2008ulg; @Lee.2010u1; @Cannas.2019], but seldom confidence interval coverage or the calibration of estimated standard errors against true sampling variability; on the applied side, fewer than a quarter of machine learning propensity score applications conduct any sensitivity analysis [@leite2025]. This study therefore treats inference quality as a primary outcome alongside bias and balance: every method is evaluated on the coverage of its nominal 95% intervals and on the ratio of its average estimated standard error to the empirical sampling variability, under a fixed-population design that makes those quantities interpretable.

\newpage

## Simulation Study

This study is a statistical simulation that evaluates ten propensity score estimation methods --- logistic regression, a classification tree (CART), bagged CART, random forest, gradient boosted trees (GBM), Bayesian additive regression trees (BART), a Super Learner ensemble, and neural networks with one, two, and three hidden layers (NN-1, DNN-2, DNN-3) --- in recovering the ATE using propensity score weighting. We specifically examine the impact of different data conditions (i.e., low vs. high-dimensional context) and the inclusion of complexities in the population treatment and outcome models on the estimated ATE, covariate balance, and the quality of the resulting statistical inference. The choice of these specific conditions is motivated by the need to understand how the performance of these techniques varies across diverse data structures, which is crucial for determining their applicability and robustness in real-world scenarios. Additionally, assessing the impact of complexities in the treatment and outcome models provides insights into the strengths and limitations of each technique, guiding researchers in selecting the most appropriate method for their studies.

Our study uses a Monte Carlo simulation design informed by previous propensity score estimation studies conducted by @Setoguchi.2008ulg, @Lee.2010u1, @Cannas.2019, @keller2015neural, @collier2021estimating. The simulation code was originally informed by the publicly available replication code of @Cannas.2019 and has been substantially rewritten and verified for this study; it follows best practices in statistical simulation as outlined by @Morris.2019abs, and the complete code, with an executable audit of the data-generating process, is available in the study repository. In the following sections, we detail the simulation design, including the data generation process, estimands of interest, specific method specifications, and performance measures.

### Data-Generation Mechanisms

The data generation mechanism for our simulation was informed by a large-scale evaluation of a national college-access intervention that we conducted using high-dimensional administrative data. We based the coefficient values of the covariates and error variances on the standardized results of that evaluation. By basing our simulation on real-world data, we aim to address the limitations of statistical simulations that may not generalize well to real datasets [@huber2013performance].

In summary, data for each design cell are generated in five steps: (1) draw and fix the cell's population --- the covariate correlation matrix, covariate roles, model coefficients, complexity terms, and calibrated intercept --- under a deterministic seed; (2) generate $n = 10{,}000$ correlated covariates of mixed types from that population; (3) compute each unit's true propensity score and assign treatment by Bernoulli draw; (4) generate the continuous outcome with a constant treatment effect of 0.3; and (5) repeat steps 2--4 for each of 1,000 replications with the population held fixed. Each step is detailed below.

#### Covariates

We used a programmatic approach to generate $(X_1,X_2,X_3,\dots,X_p)$ correlated covariates. Specifically, we simulated three covariate conditions with varying covariates generated, with $p = 20, 100, 200$. The 20 covariate conditions represented the average number of covariates used in typical propensity score analyses in educational research [@huber2013performance; @Powell.20204oe]. The remaining two conditions, with $p = 100$ and $p = 200$, were chosen as they represent high-dimensional datasets commonly encountered in education research [@grimmer2015we; @Song.2020tde; @Einav.2014; @Bird.2022gc4; @Hill.201183].

Much of the literature on propensity score simulation generates uncorrelated covariates [@Setoguchi.2008ulg; @Cannas.2019; @Lee.2010u1]. However, this does not represent the correlated variables typically found in administrative data. To address this, all covariates were generated from random draws from a multivariate normal distribution with $\mu=0$ and $\sigma=1$ using the mvrnorm function from the MASS package in R [@ripley2013package], with a correlation matrix whose off-diagonal entries were drawn from a uniform distribution on (-0.3, 0.3) and which was then adjusted to the nearest positive-definite matrix. By inducing bounded correlations that straddle 0, we aim to circumvent potential issues that may arise when correlations approach -1 or 1, such as collinearity. Because a valid correlation matrix cannot sustain arbitrarily many strong pairwise correlations as the number of covariates grows, the positive-definiteness adjustment attenuates the drawn correlations increasingly with $p$: the realized correlations span approximately $\pm.29$ (mean $|r| = .13$) at $p = 20$ but $\pm.21$ (mean $|r| = .06$) at $p = 200$, while preserving the drawn correlation structure. The higher-dimensional conditions therefore feature many weakly correlated covariates --- itself characteristic of high-dimensional administrative data.

In order to better mimic the actual structure of variables in administrative data sets and to preserve the correlation structure, we used the inverse cumulative distribution function (CDF) approach [@li2021improving] to convert the initial random pull of covariates from the multivariate normal distribution to three distributions: half of the covariates were kept as normal with a mean of 0 and standard deviation of 1, one quarter were converted to uniform with a range of 0 to 1, and the remaining quarter were converted to Bernoulli with a probability of success set at 0.5. The purpose of this step is to create more realistic and heterogeneous data that reflects the distribution of variables typically observed in real-world datasets, while maintaining the same correlation structure. Although some correlations may be slightly suppressed, they should be equal on average across the simulation. The covariates were constructed such that half affected both treatment and outcome (i.e., confounders), a quarter affected treatment only, and the remaining quarter affected the outcome only. This approach is in line with other propensity score simulation studies [@Setoguchi.2008ulg; @Cannas.2019; @Lee.2010u1].

#### Sample Size

We used a fixed sample size of 10,000 for all conditions. This sample size is larger than typical in many propensity score simulation studies (n = 500 to 2,000). The choice of this sample size is motivated by our goal to mimic real-world high-dimensional administrative data. Additionally, it was necessary to have a sufficiently large sample size to obtain accurate estimates, particularly when employing computationally intensive methods.

#### Population Treatment Assignment Models

We generated the population treatment assignment models using standard logistic regression. These models can also be referred to as population propensity score models, as they generate the true probability of being assigned to treatment. The following logistic regression model represents the general structure of these models.

$$
e(x)=Pr(Z = 1|X) = (1 + exp(-(\beta_0 + \sum_{i=1}^{p} \beta_i X_i)))^{-1}
$$ {#eq-popps}

$X$ represents covariates and their associated coefficients ($\beta$). The output, $Pr(Z = 1|X)$ is the probability of assignment to the treatment condition (i.e., the propensity score, $e(x)$). The coefficients, $\beta_i$, were randomly assigned values between -0.4 and 0.4, which were based on the corresponding standardized coefficient values in that evaluation; $\beta_i = 0$ for the quarter of covariates related only to the outcome, which do not enter the treatment model. The intercept, $\beta_0$, was then calibrated within each population --- by root-finding on a large seeded calibration sample drawn through the same covariate pipeline --- so that the marginal probability of treatment equals 0.50 in every design cell.

Importantly, all population parameters --- the covariate correlation matrix, the assignment of covariates to their confounder, treatment-only, and outcome-only roles, the model coefficients, and the selection of covariates carrying quadratic and interaction terms in the complex scenarios --- were drawn once per simulation condition and held fixed across all replications; only the data were redrawn. This design follows the convention in the propensity score simulation literature of sampling repeatedly from a single fixed population [@Setoguchi.2008ulg; @Cannas.2019], and it is what allows the confidence interval coverage and standard error results reported below to be interpreted as properties of the estimators under repeated sampling. Drawing the coefficients from ranges calibrated to a real evaluation, rather than fixing them to a single set of published constants, preserves the benefit that each covariate condition represents a distinct empirically grounded population.

We generated two population treatment assignment models, Base(T) and Complex(T). To add complexity to the complex models, we included interactions (non-additivity) and higher-order terms (non-linearity) for the confounders and covariates related to treatment.

**Base(T)**: includes only main effects, which assumes a linear relationship between covariates and treatment assignment. We consider this the base model, which is expressed using the following formula:
...
**Complex(T)**: our simulation's most complex population treatment assignment model. To construct it, we sampled half of the covariates in the treatment model and added, for each sampled covariate $X_j$, a quadratic term and an interaction with the adjacent sampled covariate $X_{j+1}$, each reusing the covariate's main-effect coefficient. This design aims to account for complex relationships between covariates and treatment assignment while keeping the number of complexity terms proportional to the number of covariates.

$$
e(x)^{Complex} = \left(1 + exp\left(-\left(\beta_0 + \sum_{i=1}^{p} \beta_iX_i + \sum_{j=1}^{J} \beta_jX_j^2 + \sum_{j=1}^{J-1} \beta_jX_jX_{j+1}\right)\right)\right)^{-1}
$$ {#eq-psD}

where $X_1, \dots, X_J$ denote the sampled covariates in their sampled order, so that the model adds $J$ quadratic terms and $J-1$ adjacent-pair interactions to the linear predictor before the logistic transformation.

Using the propensity scores generated from these population selection models, we generated a binary treatment assignment variable by comparing an individual's true propensity score to a random draw from a uniform distribution between 0 and 1. If the propensity score of an individual was higher than the random draw of the uniform distribution, the individual was assigned to treatment such that $Z = 1$, otherwise $Z = 0$. Because the intercept is calibrated, every population has a marginal treatment rate of 0.50, mirroring a balanced two-arm experiment and keeping treated and control sample sizes comparable across design cells. The strength of selection nevertheless varies by design: with coefficients drawn from a range that does not shrink with $p$, the variance of the true logit grows with the number of covariates, so the higher-dimensional conditions couple dimensionality with stronger selection and less overlap --- a bundle characteristic of high-dimensional administrative data, where richer covariate sets capture more of the selection process. We characterize each population's overlap alongside the results.

#### Population Outcome Models

In addition to the population treatment assignment models, we also generated two population outcome models. To vary the level of complexity between the covariates and the outcome, we included interactions and higher-order terms on covariates related to outcome and overall confounders. Adding complexities to the outcome model is relatively novel in the propensity score literature. However, it is more representative of the complex associations found in datasets in educational research.

The continuous outcome, $Y$, was generated through regression models. The values of $\alpha$ were likewise based on the calibrating evaluation, such that the intercept $\alpha_0$ equaled -0.18, and the remaining $\alpha$ coefficients received a random value between -0.2 and 0.3. $X_i$ represents $p$ covariates and their associated coefficients ($\alpha_i$). The error term $\epsilon$ was set to have a mean of 0 and a variance of 0.17, based on the error term in the calibrating evaluation. As in the treatment model, $\alpha_i = 0$ for the quarter of covariates related only to treatment assignment, which do not enter the outcome model. The binary treatment variable $Z$ was multiplied by $\gamma$, representing the population treatment effect, and set to 0.3 on the outcome scale, constant for all units and all design cells. Because the outcome's variance grows with the number of covariates and with outcome-model complexity, the corresponding standardized effect varies across design cells, from approximately 0.56 SD in the smallest, simplest condition to approximately 0.13 SD in the largest, most complex --- spanning the small-to-moderate effects typically observed in education research, where the average effect size equals 0.28 SD [@richardson2011eta]. Per-cell outcome standard deviations are reported with the population characteristics, and comparisons of power across conditions should be read with this gradient in mind. Because the treatment effect is constant across units, the ATE coincides with the ATT and with every weighted average treatment effect; the results therefore isolate estimation performance from the questions of estimand definition that arise under effect heterogeneity and limited overlap, consistent with the constant-effect convention of the comparison literature [@Setoguchi.2008ulg; @Cannas.2019].
...
**Base(Y)**: the base model; includes only main effects, which assumes a linear relationship between covariates and outcomes. We consider this the base model, which is expressed using the following formula:
...
$$
Y^{Complex} =  Y^{Base} + \sum_{j=1}^{J} \alpha_jX_j^2 + \sum_{j=1}^{J-1} \alpha_jX_jX_{j+1}
$$ {#eq-outcome_d}

where, as in the treatment model, $X_1, \dots, X_J$ denote the sampled outcome-model covariates in their sampled order.

### Simulation Scenarios

In total, we tested three different covariate conditions (p = 20, 100, 200) with varying levels of complexity in the treatment and outcome models (Treatment population model = Base, Complex; Outcome population model = Base, Complex). We evaluated ten propensity score estimation methods: logit, CART, bagged CART, random forest, GBM, BART, Super Learner, NN-1, DNN-2, and DNN-3. The sample size was fixed at 10,000. By fully crossing all these conditions, we tested 120 scenarios; each simulated 1,000 times. One thousand replications matches or exceeds the comparison literature [@Setoguchi.2008ulg; @Cannas.2019] and was chosen with the precision of the coverage estimates in mind: the Monte Carlo standard error of an estimated coverage rate is at most $\pm 1.6$ percentage points (and $\pm 0.7$ at the nominal 95% level), while Monte Carlo error in the bias estimates is negligible relative to the effects of interest [@Morris.2019abs]. Monte Carlo uncertainty is reported alongside the results.

### Propensity Score Estimation Methods

Using the simulated data, we estimate propensity scores using standard logistic regression and popular machine learning techniques commonly used in the propensity score estimation literature. This estimation includes a CART, bagged-CART, random forest, gradient boosted trees, Bayesian additive regression trees, a Super Learner ensemble, and neural networks of increasing depth. The propensity scores are estimated using all $p$ covariates as main effects in all cases. This scenario is unique in that all confounders are included in the propensity score estimation, which is not typically the case in practice. However, given our interest in evaluating the ability of these methods to recover the true unbiased treatment effect, this approach is well-suited. Future research should examine the performance of these methods when a subset of confounders is omitted from the propensity score estimation model.

A guiding principle of this study is to evaluate each method as an applied researcher would actually encounter it, rather than under study-specific tuning. This choice reflects how these tools are used in practice: in Stata, the official `teffects ipw` command estimates the treatment model with logistic regression by default, and its only alternatives are probit specifications [@statacorp2025causal] --- machine learning propensity scores are effectively unavailable to Stata users without community add-ons. In R, the *WeightIt* package [@greifer2026weightit], the standard interface for propensity score weighting, defaults to logistic regression, and its boosted-tree method carries the package's default configuration. Because estimating the propensity score is a step that precedes --- and is unaffected by --- whether the score is subsequently used for weighting or matching, its companion package *MatchIt* [@ho2011matchit] is an equally relevant reference for estimation defaults: its default distance is a logistic model, and where it offers machine learning alternatives it passes package defaults through directly --- classification trees via *rpart* with in-sample predictions, and random forests via *randomForest* with out-of-bag predictions --- while its neural network option will not run until the user chooses the hidden-layer size. A recent systematic review found that applications of machine learning for propensity score estimation rarely report --- and rarely perform --- hyperparameter tuning [@leite2025]. Accordingly, logistic regression, GBM, BART, CART, and random forest are specified here exactly as their default software implementations deliver them, with bagged CART following @Lee.2010u1 since no major package offers it; the Super Learner and deep neural networks --- for which the software itself requires the user to make architectural choices --- represent the more deliberately configured end of the spectrum. A single principle governs the one place the packages' defaults diverge: any learner that is tuned at all is tuned toward prediction, so that no method is selected on the covariate-balance metrics this study uses for evaluation. For boosting, where *MatchIt* and *WeightIt* share the same hyperparameters but select the number of trees by cross-validated deviance and covariate balance respectively, this principle rules out the balance criterion; for computational feasibility the prediction-based selection uses a single validation split rather than *MatchIt*'s five-fold cross-validation --- the study's one stated deviation from a software default, detailed in the GBM entry below. The balance-stopping alternative is a promising variant this design does not test. Notably, the most recent comparable simulation reports only that *gbm* "default values" were used without stating them [@liJSSM2025]; every configuration value in this study is reported.

Penalized logistic regression (lasso, ridge, and elastic net), which both packages offer, appears here as a component of the Super Learner library rather than as a standalone method. Two families of related methods fall outside this study's scope. Balancing-weight approaches --- entropy balancing, energy balancing, and optimization-based weights --- solve directly for weights that balance covariates without estimating a propensity score model, and covariate balancing propensity scores blend the two strategies; these are distinct estimation philosophies deserving their own comparison. Similarly, doubly robust estimators that combine the propensity score with an outcome model are addressed in the Discussion as an important direction the present singly robust design informs.

Here we present the specification of each method used to estimate propensity scores:

-   **Logistic regression**: standard logistic regression with only main effects, using the R *glm* command [@team2009language].

-   **Classification and Regression Trees (CART)**: using the R *rpart* package [@therneau2015package] with default parameters and recursive partitioning, with in-sample predicted probabilities --- the same behavior *MatchIt* delivers for `distance = "rpart"` [@ho2011matchit].

-   **Bagged CART**: bootstrapped aggregated classification trees using the R *ipred* package [@peters2009package]. We changed the default parameter to use 100 bootstrap replicates to align with the @Lee.2010u1 simulation. Propensity scores are the out-of-bag class probabilities, so each student's score is predicted only by trees whose bootstrap sample excluded that student; this avoids the overfitting that arises when ensemble predictions are taken on the training data itself.

-   **Random Forest**: parallel tree generation on subsamples based on randomly selected covariates using the R *randomForest* package with default parameters [@liaw2002classification]. As with bagged CART, propensity scores are the out-of-bag vote proportions rather than in-bag predictions --- the default behavior of *MatchIt*'s `distance = "randomforest"` option [@ho2011matchit].

-   **Gradient Boosted Trees (GBM)**: boosted classification trees under the hyperparameter configuration shared by *MatchIt* and *WeightIt* as their default [@ho2011matchit; @greifer2026weightit]: Bernoulli deviance and 10,000 trees of depth three with a shrinkage rate of 0.01, each fit to the full sample (no subsampling), using the *gbm* package [@ridgeway2026gbm]. The number of boosting iterations --- boosting's key regularization parameter --- is selected on a single random 20% validation split, one of the three selection rules built into *gbm*; this is the study's one stated deviation from a software default, adopted for computational feasibility (one 10,000-tree fit rather than the six that *MatchIt*'s five-fold cross-validation requires) and mirroring the validation-based early stopping used for the neural networks. Selection remains prediction-based --- *WeightIt*'s alternative, the covariate-balance stopping rule of @McCaffrey.2004pse, is not used so that no learner is selected on the balance metrics this study uses for evaluation. Predictions are in-sample, as in *MatchIt*.

-   **Bayesian Additive Regression Trees (BART)**: a Bayesian sum-of-trees ensemble in which each tree is constrained by a regularization prior to act as a weak learner [@chipman2010bart], with a strong track record in causal inference applications [@Hill.201183]. Propensity scores are the posterior-mean probabilities exactly as delivered by *WeightIt* with `method = "bart"` and all defaults [@greifer2026weightit], which wraps the *dbarts* implementation.

-   **Super Learner**: a stacked ensemble [@vanderLaan2007; @Pirracchio.201523m] combining logistic regression, lasso-penalized logistic regression (*glmnet*), gradient boosted trees (*xgboost*), random forest (*ranger*), and a single-hidden-layer neural network (*nnet*), with the convex combination weights estimated by non-negative least squares on five-fold cross-validated predictions using the R *SuperLearner* package and its default wrappers for these learners. Propensity scores are the cross-validated ensemble predictions, mirroring the out-of-bag convention used for the tree ensembles.

-   **Single-Layer Neural Network (NN)**: we used a simple three-layer NN that consisted of an input layer, one hidden layer, and an output layer. The input layer consisted of $p$ covariates. The hidden layer comprised hidden neurons equal to two-thirds of the number of input covariates, a common practice in NN applications [@boger1997knowledge]. We used a ReLU activation function in the hidden layer for computational efficiency. The ReLU function introduces non-linearity into the network and allows it to learn more complex relationships between input covariates and output. In the output layer, we used a sigmoid activation function to obtain probabilities bounded between 0 and 1. The sigmoid function smoothly maps any input value to a probability between 0 and 1, making it suitable for binary classification tasks. The NN was trained and fit using the R Keras package [@chollet2015keras], which interfaces with Python.

-   **Deep Neural Network (DNN-2)**: we used a four-layer deep neural network with an input layer, **two** hidden layers, and an output layer. The input layer consisted of $p$ covariates, and each hidden layer comprised hidden neurons equal to two-thirds of the number of input covariates. Similar to the NN, we used the ReLU activation function in the hidden layers. In contrast, the output layer consisted of a single neuron with a logistic activation function, which provides bounded probabilities between 0 and 1. We trained and fit the DNN-2 using the R Keras package [@chollet2015keras].

-   **Deep Neural Network (DNN-3)**: The last neural architecture we tested was a five-layer deep neural network with an input layer, **three** hidden layers, and an output layer. A more complex DNN structure, such as this one, can be beneficial when dealing with high-dimensional and non-linear data, as it enables the network to learn more complex and intricate patterns, ultimately improving its predictive performance. The output layer was a single neuron with a logistic activation function to produce bounded probabilities between 0 and 1. The ReLU activation function was also used in the hidden layers. The DNN-3 was also trained and fit using the R Keras package [@chollet2015keras], which interfaces with Python.
...
Various techniques for hyperparameter tuning have been developed to optimize the model's performance, such as k-fold cross-validation and grid search. Consistent with this study's principle of evaluating methods as practitioners encounter them, we did not tune the networks. Optimization used the ADAM optimizer [@kingma2014adam] at its default base learning rate of 0.001. ADAM adapts the step size for each weight based on the history of its gradients, which makes training considerably less sensitive to the base learning rate than classical gradient descent, although it does not eliminate the learning rate as a hyperparameter.

The neural network approaches we tested (NN-1, DNN-2, DNN-3) were trained using a random 80/20 split, with approximately 80% of the sample used for training and 20% for validation. The networks were trained to minimize the binary cross-entropy loss function, the standard objective in binary classification, which measures the discrepancy between predicted probabilities and observed treatment status. Training stopped early when the *validation* loss failed to improve by at least 0.001 over five consecutive epochs, and L2 regularization with a value of 0.01 was applied to penalize large weights. These precautions minimized the chances that the neural networks overfit the data. Input covariates were standardized using training-set means and standard deviations before being passed to the networks.

ReLU activation was used in the hidden layers for computational efficiency, and a logistic activation function was used in the output layer to generate probabilities between 0 and 1. The training was conducted for up to 100 epochs (subject to early stopping) with a batch size of 64. The choice of hyperparameters was consistent across all three methods to ensure a fair comparison of their performance. The use of the ADAM optimizer, early stopping, and L2 regularization all contributed to the efficient training of the neural networks and optimizing their performance.

### Estimating Treatment Effects - Propensity Score Weighting

To estimate the causal effect of the treatment on the outcome, we used a propensity score weighting approach, specifically the inverse probability of treatment weighting (IPTW) [@Lee.2010u1]. This method involves reweighing observations based on their estimated propensity score to achieve a covariate balance between the treated and untreated units. The weight for each treated observation was equal to $1/e_i(x)$, and for untreated units, the weight was $1/(1-e_i(x))$. The estimated propensity score, $e_i(x)$, was determined for each unit $i$. Because tree ensembles can produce out-of-bag probabilities of exactly 0 or 1, which yield undefined weights, estimated propensity scores were bounded at $[10^{-6}, 1-10^{-6}]$; this guard is far outside any substantively meaningful range, so extreme-weight behavior remains fully visible in the results. In accordance with the different methods given above, this weighting scheme was used to estimate the ATE. As a sensitivity analysis for extreme weights, we additionally re-estimated the ATE with weights truncated at their 1st and 99th percentiles [@Lee.2011tr]. Notably, no major software path applies trimming by default, and most applied studies report none [@leite2025]; trimming is therefore evaluated here as the literature's most commonly recommended *remedial* step rather than as part of the primary practitioner workflow being simulated.

We used the R *survey* package to calculate the ATE [@lumley2020package]. The outcome variable ($Y$) was predicted using only the binary treatment assignment variable, with the ATE weights included. The ATE point estimate, as well as the robust standard error of the ATE, were saved.
...
Given these debates, it is vital to choose an appropriate standard error estimator based on the specific goals of the analysis and the trade-off between computational feasibility and statistical accuracy. Robust standard errors are preferred in this context because they account for the potential biases and uncertainties introduced by the propensity score estimation process, as well as any potential model misspecification. By providing more accurate estimates of the variability in the treatment effect, robust standard errors can lead to more reliable hypothesis testing and confidence interval estimation. To account for the uncertainty in the estimated propensity score, we used the "robust" standard errors calculated from the R survey package, as recommended by @robins2000marginal. All reported standard errors, p-values, and confidence intervals derive from this same sandwich estimator. To quantify how much the choice of estimator matters, we additionally saved the model-based standard error from a weighted least squares regression, which treats the weights as precision weights, and we compare both against the empirical Monte Carlo standard deviation of the ATE estimates within each condition.

### Performance Metrics

To gauge the performance of each propensity score estimation method, we used the following metrics, which are widely used to assess model performance in the propensity score simulation literature [@Setoguchi.2008ulg; @Cannas.2019; @Lee.2010u1]:
...
-   **Average Standardized Absolute Mean Distance (ASAM)**: is a measure of covariate balance. After weighting, the absolute value of the standardized difference in weighted means between the treated and untreated groups was calculated for each covariate and averaged across all covariates. Each difference is standardized by the covariate's standard deviation among treated units in the *unweighted* sample, so that the denominator is constant across methods and ASAM values are directly comparable between them. Lower ASAM values indicate better covariate balance. We additionally report the maximum standardized difference across covariates, which identifies methods that balance well on average while leaving individual covariates badly imbalanced.
...
-   **Weights**: a known issue with PSW is that extreme weights can result in biased treatment effects. Therefore, we examine the distribution of the estimated weights, including the maximum weight and the effective sample size implied by the weights [@Kish.1965]. We also report the ATE, its standard error, and coverage under weights truncated at the 1st and 99th percentiles [@Lee.2011tr].

-   **Standard Error Calibration**: the ratio of the average estimated standard error to the empirical Monte Carlo standard deviation of the ATE estimates within a condition. Values below one indicate that the estimator understates the true sampling variability, which translates directly into under-coverage of the nominal 95% confidence interval.

### Software and Reproducibility

The simulation is implemented in R (version 4.5) and orchestrated with the *SimDesign* framework [@chalmers2020simdesign], which manages the generate--analyse--summarise pipeline, replication-level random seeds, and parallelization. Propensity scores are estimated with the packages documented for each method above; treatment effects and standard errors are estimated with the *survey* package [@lumley2020package]; and the neural networks are trained through the R *keras* interface [@chollet2015keras] to TensorFlow (version 2.16). Within each design cell, all population parameters are generated under a deterministic, cell-derived seed with a pinned random number generator, so that every method --- and any independent replication of this study --- faces byte-identical populations regardless of the computing platform or parallelization scheme; only the replication-level data draws vary. The simulations were executed on high-memory multicore nodes, and the complete pipeline was additionally validated end-to-end on an independent cloud environment before the production run. All code --- including the runners, an executable audit of the data-generating process's statistical properties, and a scripted environment build --- is available in the study repository. \newpage

## Results

In this section, we present the results of our Monte Carlo simulation study, which compares the performance of various methods in estimating the ATE through Propensity Score Weighting. The methods include our DNN-based approach, traditional logistic regression, and other commonly used machine learning-based approaches such as CART, bagged-CART, and random forest. The simulation aimed to evaluate the accuracy of these methods in producing a propensity score used in estimating the ATE (ATE = 0.3) with IPTW, under different levels of covariate conditions (p = 20, 100, 200) and with varying levels of complexity in the treatment and outcome models (Treatment population model = Base, Complex; Outcome population model = Base, Complex).

The results are presented in four main parts. First, we assess covariate balance through the average standardized absolute mean difference (ASAM). Second, we examine the bias in the ATE estimation. Third, we evaluate the variability of the ATE estimation through the standard error (SE), MSE, and 95% confidence interval coverage. Finally, we present the mean IPTW weights and discuss the results regarding statistical power.

### Covariate Balance

We begin by considering the performance of a commonly used and directly observable metric for deciding which method to use, covariate balance [@Lee.2010u1]. Covariate balance is assessed using the average standardized absolute mean difference (ASAM) between treated and untreated units. Maintaining a covariate balance reduces the risk of bias in the estimated treatment effect. In our simulation, we focused on assessing balance through ASAM and took the average ASAM for all covariates in the propensity score estimation model. However, in practice, each covariate should be assessed separately. A mean ASAM value of 0 indicates a perfect balance in the mean between treated and untreated units across all covariates. We consider two commonly used thresholds for deciding whether adequate covariate balance has been achieved: the more typical 0.2 thresholds [@Imbens.2015] and the more stringent threshold of 0.1 [@austin2009balance].
...
In this section we present the evaluation of the bias in the ATE across the different population treatment and outcome model conditions to understand the overall performance of each method. Bias is the difference between the estimated and true ATE (set to 0.3). In other words, it measures how much, on average, a method tends to over- or under-estimate the true ATE. @fig-bias displays our results for the bias of the ATE by method and covariate conditions (p = 20, 100, 200). The solid dots represent the mean bias, and the extending lines are the associated Monte Carlo 95% confidence intervals.
...
In the first panel of @fig-ate-var, we present the standard error (SE) results of the estimated ATE by covariate condition. In the low-dimensional condition, all methods produced small standard errors. However, as the number of covariates increased, the SE of logistic regression and NN-1 increased significantly. At the same time, other machine learning approaches, such as CART and random forest, had smaller SEs (SE = 0.22 and 0.21, respectively). Despite the differences in the SE, all methods produced narrow 95% Monte Carlo confidence intervals for the estimated ATE, indicating high precision in the ATE estimate.
...
In the upper-left panels of @fig-ate-var-cond, we present the results when only the main effects are included in both the population treatment and outcome models. Regardless of the covariate condition, all methods performed well and produced low values for both the SE and MSE of the estimated ATE. This result indicates that the estimated ATE is precise and accurate when only the main effects are present in both models.

###### Complex - Population Outcome and Selection Models

As the complexity of the population treatment and outcome models increased, we observed a substantial increase in the SE and MSE of the estimated ATE. In the 100-covariate condition, logistic regression had a high MSE value (MSE = 7.51). However, when the number of covariates increased to 200, logistic regression resulted in extremely high SE and MSE values (SE = 4.00; MSE = 115.1). In contrast, our DNN-2 and DNN-3 approaches had significantly lower MSE values, with MSE values below 2, which were nearly 60 times lower than the MSE value for logistic regression in the high-dimensional condition.

###### Deviations - Population Outcome and Selection Models

As we observed with the ATE bias evaluation results, both the SE and MSE were more sensitive to complexities in the outcome model than when complexities were only included in the selection model. When complexities were only included in the outcome model, we found that the CART and random forest methods produced larger MSE values than all other methods. In the highest covariate condition, all other methods, including logistic regression, produced MSE values well below 2.5.

#### 95% Confidence Interval Coverage

Finally, in the third panel (@fig-ate-var-3), we present our findings for the 95% confidence interval (CI) coverage rate of the ATE across different population outcome models. The dashed line indicates the desired coverage rate of 95%. This rate assesses how well each method captures the true ATE of 0.3 within the estimated 95% confidence interval. A good estimator has high coverage and reliably captures the true population ATE within the interval.
...
One possible scenario that could lead to low coverage is that the estimated standard errors are biased away from the empirical standard errors. To analyze whether our estimated standard errors are biased away from the empirical standard errors, we computed the relative percent error in the standard errors using the following @eq-empse:
...
When evaluating the IPTW weights by the different complexities in the population outcome and selection models, we found that all methods produced acceptable weights below 2 (@fig-weights-cond) regardless of the condition. The only exception to this was logistic regression, which produced extreme weights whenever complexities were introduced into the population selection model. This effect was not seen when the base selection model only included the main effects.
...
<!-- EMPIRICAL APPLICATION (co-author): JEBS convention is a real-data
application as its own section here, after the simulation — Data (+Variables)
and Results subsections, with a sensitivity analysis (see
research/jebs_structure_survey.md; ECLS-K:2011 or HSLS:09 are the
JEBS-conventional public candidates). -->
...
The purpose of our study was to address the limitations in the existing literature on propensity score estimation in high-dimensional data settings. We compared the performance of a DNN approach with traditional logistic regression and other machine learning-based methods, including CART, bagged-CART, and random forest, to provide valuable insights into these methods' accuracy in estimating the population ATE. We evaluated these methods' performance under different covariate conditions and when complexities were introduced into the treatment selection and outcome models. This evaluation is critical given recent simulation studies have found that the presence of confounding variables and complexities in the treatment selection and outcome models can significantly impact the accuracy of the estimated ATE [@Cannas.2019].

Our results suggest that the choice of method for propensity score estimation should be based on the context and dimensionality of the data. In low-dimensional settings (p=20), we found that most methods, including logistic regression, produced accurate ATE estimates with little to no bias. However, as the number of covariates increased, the magnitude of bias also increased, with logistic regression overestimating the ATE. The other machine learning methods we tested produced less biased results, including CART, bagged-CART, random forest, and the neural network-based approaches (NN-1, DNN-2, DNN-3).

In high-dimensional settings (p=200), logistic regression severely overestimated the true ATE. On the other hand, the machine learning methods we tested, including the DNN-2 approach, could estimate the true ATE with significantly less bias and better covariate balance. When choosing between DNNs and bagged-CART, applied analysts should consider factors such as data complexity, presence of non-linear relationships, computational resources, and the desired level of interpretability. DNNs may be better suited for cases with more complex relationships, while bagged-CART may be preferred when interpretability is crucial and computational resources are limited.

Traditional logistic regression may not be suitable for estimating the ATE in high-dimensional covariate conditions, especially in the presence of non-linearity and non-additivity. Machine learning-based approaches, particularly DNNs, are promising alternatives. Our findings align with other studies and emphasize the importance of the bias and variability of the estimated ATE when selecting the optimal estimation method. Our study extends the literature by testing various methods with more complex and higher dimensional data than previous studies on propensity score estimation [@Setoguchi.2008ulg; @Lee.2010u1; @Cannas.2019].

Like other simulation studies, our study has limitations in terms of generalizability, meaning that our findings may not necessarily generalize outside the tested scenario. Although this simulation is calibrated to a real-world education evaluation, making it more generalizable than simulations based on arbitrary data pulls, further research is still needed to evaluate the performance of the DNN-2 approach and other machine learning methods in different settings, including real-world datasets. To address this, we plan to include a real-world data example in our publication, demonstrating how the DNN approach and other methods can be used to estimate treatment effects using actual data.

Another limitation of our study relates to inference. Although the DNN approach and bagged-CART produced unbiased estimates and outperformed the logistic model in the high-dimensional context, their coverage rates were suboptimal. This performance is likely due to the model-based SEs underestimating the true empirical SE. To address this, we plan to conduct a follow-up analysis using bootstrapping to estimate the SEs. Bootstrapping does not require assumptions of normality, which may be violated in high-dimensional and complex data, leading to poor estimation of SEs even with robust sandwich estimation [@efron1986bootstrap]. This fact highlights the importance of considering the bias introduced in estimating SEs in any simulation study using propensity score estimation with high-dimensional data. The literature around variance estimation in propensity score weighting is heavily debated, and our study contributes to this ongoing debate.
...
## Appendix A: Neural Network Architectures {#sec-appendix-nn}
