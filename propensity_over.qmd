...
A guiding principle of this study is to evaluate each method as an applied researcher would actually encounter it, rather than under study-specific tuning. This choice reflects how these tools are used in practice: in Stata, the official `teffects ipw` command estimates the treatment model with logistic regression by default, and its only alternatives are probit specifications [@statacorp2025causal] --- machine learning propensity scores are effectively unavailable to Stata users without community add-ons. In R, the *WeightIt* package [@greifer2026weightit], the standard interface for propensity score weighting, defaults to logistic regression, and its boosted-tree method carries the package's default configuration. A recent systematic review found that applications of machine learning for propensity score estimation rarely report --- and rarely perform --- hyperparameter tuning [@leite2025]. Accordingly, logistic regression and GBM are specified here exactly as their default software implementations deliver them; the tree-based methods follow the configurations established in the simulation literature [@Lee.2010u1; @Cannas.2019], with out-of-bag predictions as documented good practice; and the Super Learner and deep neural networks represent the more deliberately configured end of the spectrum. Where a configuration goes beyond a software default, I state so explicitly.

Two families of related methods fall outside this study's scope. Balancing-weight approaches --- entropy balancing, energy balancing, and optimization-based weights --- solve directly for weights that balance covariates without estimating a propensity score model, and covariate balancing propensity scores blend the two strategies; these are distinct estimation philosophies deserving their own comparison. Similarly, doubly robust estimators that combine the propensity score with an outcome model are addressed in the Discussion as an important direction the present singly robust design informs.
...
-   **Random Forest**: parallel tree generation on subsamples based on randomly selected covariates using the R *randomForest* package with default parameters [@liaw2002classification]. As with bagged CART, propensity scores are the out-of-bag vote proportions rather than in-bag predictions.
