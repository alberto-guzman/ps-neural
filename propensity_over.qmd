...
-   **Gradient Boosted Trees (GBM)**: boosted classification trees under the default configuration shared by *MatchIt* and *WeightIt* [@ho2011matchit; @greifer2026weightit]: Bernoulli deviance and 10,000 trees of depth three with a shrinkage rate of 0.01, each fit to the full sample (no subsampling). The packages differ only in how the number of boosting iterations is selected --- *WeightIt* uses the covariate-balance criterion of @McCaffrey.2004pse, while *MatchIt* uses five-fold cross-validated deviance. I use the cross-validation rule so that every learner in this comparison is configured for prediction and none is selected on the balance metrics used for evaluation; the implementation is therefore identical to *MatchIt*'s `distance = "gbm"`, with in-sample predictions.
