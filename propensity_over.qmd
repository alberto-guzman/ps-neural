...
-   **Gradient Boosted Trees (GBM)**: boosted classification trees exactly as *MatchIt* delivers them for `distance = "gbm"` [@ho2011matchit]: Bernoulli deviance, 10,000 trees of depth three with a shrinkage rate of 0.01, and the number of boosting iterations selected by five-fold cross-validated deviance, with in-sample predictions. These are the same hyperparameters *WeightIt* uses by default; the two packages differ only in the selection rule --- cross-validated prediction error versus the covariate-balance criterion of @McCaffrey.2004pse. I use the prediction-based rule so that every learner in this comparison is configured for prediction, with none given access to the balance metrics used for evaluation.
