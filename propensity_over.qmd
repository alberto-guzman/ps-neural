...
-   **Gradient Boosted Trees (GBM)**: boosted classification trees exactly as an applied researcher obtains them from the *WeightIt* package with `method = "gbm"` and all defaults [@greifer2026weightit]: 10,000 trees of depth three with a shrinkage rate of 0.01, with the number of boosting iterations selected by minimizing the average absolute standardized mean difference between groups --- the balance-stopping rule introduced for propensity score estimation by @McCaffrey.2004pse.
