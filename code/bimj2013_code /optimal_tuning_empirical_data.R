...
for (i in 1:length(unique(data$fold))) {
 ...
 cp2[which.max(rpartacc)]#0.005
 cp2[which.min(asam_am_tree)]#0.0025
 cp2[which.min(asam_aw_tree)]#0.0025
...
for (i in 1:length(unique(data$fold))){ 
 ...
 ##############################
 # load("results/tuning_empirical_results.Rdata")
 # pdf(paste("results/Fig1_Tuning_empirical.pdf"),width=12,height=9)
 # define window 
 par(mfrow=c(2,3))
  # plot rf
 par(mar = c(4,4,2,4))
...
#abline(v=3,col="red",lty="dotted")#default value of mtry
...
segments(x0=0.01,y0=0,x1=0.01,y1=15,lty="dotted")
#points(mtry,asam_am,col=gray(0.8),pch=17)
