...
s1 <- simsum(data = res, estvarname = "ATE", true = 0.3, se = "ATE_se", methodvar = "method", by = c("p"), ref = "logit", x = TRUE)
s2 <- simsum(data = res, estvarname = "ATE", true = 0.3, se = "ATE_se", methodvar = "method", by = c("p", "scenarioT", "scenarioY"), ref = "logit", x = TRUE)
...
autoplot(s1, type = "est_ridge") + xlim(0, .4)
